...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if(app_cfg_cplt)
	{
		if(p_sys_cfg->mode == TIME)
		{
			if((g_clock[1] == p_sys_cfg->time_open_minute) && (g_clock[0] == p_sys_cfg->time_open_hour))
			{
				put_event_task_menu(EV_TIME_OPEN);
			}
			if((g_clock[1] == p_sys_cfg->time_close_minute) && (g_clock[0] == p_sys_cfg->time_close_hour))
			{
				put_event_task_menu(EV_TIME_CLOSE);
			}
		}
		else if((p_sys_cfg->mode == LIGHT) && app_sleep)
		{
